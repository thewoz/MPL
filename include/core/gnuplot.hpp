...
#ifndef _H_MPL_EXTERNAL_GNUPLOT_H_
#define _H_MPL_EXTERNAL_GNUPLOT_H_
...
namespace mpl {
  ...
  public:
    ...
    template <class T>
    void plot(const std::vector<T> & points) {
    ...
    inline void init(bool persist = true) {
        ...
        if(!pipe) {
          fprintf(stderr, "error: cannot open the pipe in mpl::gnuplot()\n");
          abort();
        }
...
#endif // _H_MPL_EXTERNAL_GNUPLOT_H_
