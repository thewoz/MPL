...
#ifndef _H_MPL_CORE_OPT_H_
#define _H_MPL_CORE_OPT_H_
...
namespace mpl {
  ...
  public:
    ...
    static bool add(const std::string & key, const std::string & description, const std::string & longDescription, int haveArgument, bool isMandatory, const std::string & value = "") {
      ...
      if(keys.size() > 2) {
        fprintf(stderr, "mpl::opt::add() error: an option can be have only one long and one short descriptor\n");
        abort();
      }
      ...
      if(find(opt.key) || find(opt.longKey)) {
        fprintf(stderr, "mpl::opt::add() error: option \"%s\" already defined\n", key.c_str());
        abort();
      }
      
      if(isMandatory && value.length() != 0) {
        fprintf(stderr, "mpl::opt::add() error: a mandatory option \"%s\" can not have a default value \"%s\"\n", key.c_str(), value.c_str());
        abort();
      }
      
      if(!haveArgument && value.length() != 0) {
        fprintf(stderr, "mpl::opt::add() error: option \"%s\" without argument can not have a default value \"%s\"\n", key.c_str(), value.c_str());
        abort();
      }
  ...
  public:
    ...
    static void init(int argc, const char * argv[]) {
      ...
      for(int i=1; i<argc; ++i) {
        ...
        if(waitForArgument) {
          ...
          if(opt::find(extractOption(argv[i]))) {
            fprintf(stderr, "mpl::opt::init() error: the '%s' option needs an argument\n", option.c_str());
            exit(EXIT_FAILURE);
          }
        ...
        if(currentOpt != NULL) {
          ...
          if(currentOpt->isInArgv){
            fprintf(stderr, "mpl::opt::init() error: the '%s' option has been set more than once\n", option.c_str());
            exit(EXIT_FAILURE);
          }
          ...
          fprintf(stderr, "mpl::opt::init() error: the '%s' option was not recognized\n", option.c_str());
          exit(EXIT_FAILURE);
        }
      ...
      if(waitForArgument) {
        fprintf(stderr, "mpl::opt::init() error: missing value for option '%s'\n", option.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static bool set(const std::string & key, const std::string & value) {
      ...
      if((optPtr = find(key)) != NULL){
        ...
        fprintf(stderr, "mpl::opt::set() error: parameter '%s' not found\n", key.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static std::string get(const std::string & key) {
      ...
      if((optPtr = find(key)) != NULL) {
        ...
        if(optPtr->haveArgument == NONE_ARGUMENT) {
          fprintf(stderr, "mpl::opt::get() error: parameter '%s' has no argument\n", key.c_str());
          exit(EXIT_FAILURE);
        }
        ...
        fprintf(stderr, "mpl::opt::get() error: parameter '%s' not found\n", key.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static cv::Range getRange(const std::string & key) {
      ...
      if(tokens.size() != 2) {
        fprintf(stderr, "mpl::opt::getRange() error: cannot parse '%s'\n", key.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static cv::Point2d getPoint(const std::string & key) {
      ...
      if(tokens.size() != 2) {
        fprintf(stderr, "mpl::opt::getPoint() error: cannot parse '%s'\n", key.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static cv::Size getSize(const std::string & key) {
      ...
      if(tokens.size() != 2) {
        fprintf(stderr, "mpl::opt::getSize() error: cannot parse '%s'\n", key.c_str());
        exit(EXIT_FAILURE);
      }
    ...
    static bool isDefined(const std::string & key) {
      ...
      } else { fprintf(stderr, "mpl::opt::isDefined() error: parameter '%s' not found\n", key.c_str()); exit(EXIT_FAILURE); }
  ...
  private:
    ...
    static bool check() {
      ...
      if(isNotOk.size() > 0){
        ...
        fprintf(stderr, "mpl::opt::check() error: the following mandatory options were not specified:\n");
