...
#ifndef _H_MPL_CORE_PROFILE_H_
#define _H_MPL_CORE_PROFILE_H_
...
#pragma omp critical
      {
        ...
        if(logTimeFile[thread] == NULL) {
          fprintf(stderr, "mpl::initTime() error: cannot open '%s': %s\n", str, strerror(errno));
          abort();
        }
      }
#else
...
#pragma omp critical
      {
        ...
        if(logMemoryFile[thread] == NULL) {
          fprintf(stderr, "mpl::initMemory() error: cannot open '%s': %s\n", str, strerror(errno));
          abort();
        }
      }
#else
...
#pragma omp critical
  ...
  static double memoryPeakGB() {
    ...
    if(file == NULL) {
      fprintf(stderr, "mpl::memoryPeakGB() error: cannot open '/proc/self/status': %s\n", strerror(errno));
      abort();
    }
    ...
    while(fgets(line, 128, file) != NULL){
      ...
      if(pch != NULL){
        ...
        sscanf(line, "%s %zu %s", dummy1, &memoryKB, dummy2);
...
private:
  ...
  static void getMemUsage(size_t & currentUsage, size_t & peakUsage) {
    ...
    if(file == NULL) {
      fprintf(stderr, "mpl::getMemUsage() error: cannot open '/proc/self/status': %s\n", strerror(errno));
      abort();
    }
