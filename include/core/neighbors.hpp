...
#ifndef _H_MPL_CLUSTER_NEIGHBORS_H_
#define _H_MPL_CLUSTER_NEIGHBORS_H_
...
namespace mpl::neighbors {
  ...
  std::vector<std::vector<size_t> > byDistance(const T1 & setA, const T2 & setB, double maxDist, std::size_t maxNeighborSize = std::numeric_limits<std::size_t>::max()){

    std::vector<std::vector<size_t> > neighbor(setA.size());

    // ciclo su tutti i punti nel primo insieme
    for(std::size_t i=0; i<setA.size(); ++i){

      std::vector<size_t> & tmpNeighbor = neighbor[i];
...
#endif // _H_MPL_CLUSTER_NEIGHBORS_H_
