...
#ifndef _H_MPL_EXTERNAL_FITS_H_
#define _H_MPL_EXTERNAL_FITS_H_
...
namespace mpl {
  ...
  namespace fits {
    ...
    bool solveField(const std::string & filename, const std::string & _output = "") {
      ...
      if(fp == NULL) {
        fprintf(stderr, "mpl::fits::solveField() error: cannot open pipe\n");
        abort();
      }
      ...
      // Chiudo la pipe
      if(pclose(fp) == -1) {
        fprintf(stderr, "mpl::fits::solveField() error: cannot close pipe\n");
        abort();
      }
