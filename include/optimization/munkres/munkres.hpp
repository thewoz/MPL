...
#include <cobbs/munkres/include/matrix.h>
...
namespace munkres {
  ...
  public:
    ...
    void solve (const TI & m, std::vector<TO> & o);
  ...
  template<typename T> template<typename TI, typename TO>
  void Munkres<T>::solve(const TI & _matrix, std::vector<TO> & matches)
  {
    ...
    for(size_t i=0; i<matrix.rows(); ++i){
      ...
      for(size_t j=0; j<matrix.columns(); ++j){
        ...
        if(matrix(i,j) == 0 &&  _matrix(i,j) != matrix.inf())
          matches.push_back(TO(i,j));
  ...
  void Munkres<T>::solve (matrix_base<T> & matrix)
  {
    ...
    if (rows != columns) {
      ...
      // in the matrix.
      matrix.resize ( size, size, matrix.inf() );
    }
