...
#ifndef _H_MPL_MATH_KALMAN_H_
#define _H_MPL_MATH_KALMAN_H_
  ...
  public:
    ...
    void drawPrediction(cv::Mat & image, int size = 30) {
      ...
      if(image.type() != CV_8UC3 && image.type() != CV_8UC1) {
        fprintf(stderr, "mpl::kalman_t::drawPrediction() error: destination image type must be CV_8UC3 or CV_8UC1\n");
        abort();
      }
