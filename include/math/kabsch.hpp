...
#ifndef _H_MPL_GEOMETRY_KABSCH_H_
#define _H_MPL_GEOMETRY_KABSCH_H_
...
namespace mpl::geometry::kabsch {
  ...
  namespace util_kabsch {
    ...
    cv::Mat _solve(int size, int dims, const double * P, const double * Q, cv::Mat & R, cv::Mat & T, double * S = NULL, double * barycenterP = NULL, double * barycenterQ = NULL, double EPS = DBL_EPSILON) {
      ...
      double * p0 = (double *) calloc((size_t)dims, sizeof(double));
      double * q0 = (double *) calloc((size_t)dims, sizeof(double));
      ...
      for(int dp=0; dp<dims; ++dp){

        const double * rowP = &P[dp*size];

        for(int dq=0; dq<dims; ++dq){
...
// compute the optimal scaling
      if(S != NULL){
        ...
        if(size > 1){
          ...
          double * RP = (double *) calloc((size_t)size, sizeof(double));

          for(int ir=0; ir<dims; ++ir) {

            memset(RP, 0, (size_t)size*sizeof(double));
