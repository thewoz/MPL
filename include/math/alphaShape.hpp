...
#ifndef _H_MPL_GEOMETRY_ALPHASHAPE_H_
#define _H_MPL_GEOMETRY_ALPHASHAPE_H_
...
inline gridKey_t makeGridKey(const cv::Point2f & point, float cellSize) {
  return { (int)(std::floor(point.x / cellSize)), (int)(std::floor(point.y / cellSize)) };
}
...
inline std::vector<std::vector<int>> extractClosedContoursByIndex(const std::vector<std::pair<int, int>> & boundaryEdges) {
  ...
  if(irregularTopology) {
    fprintf(stderr, "mpl::geometry::alphaShape() warning: irregular boundary topology (vertices with degree other than 2 exist)\n");
  }
...
std::vector<cv::Point2f> alphaShape(const std::vector<T> & points, double alpha, float pointMatchEps = utils::kPointEpsilon, float gridCellSize = 1.0f) {
  ...
  if(contours.size() > 1) {
    fprintf(stderr, "mpl::geometry::alphaShape() warning: found %zu closed contours, returning the one with the largest area\n", contours.size());
  }
...
#endif // _H_MPL_GEOMETRY_ALPHASHAPE_H_
