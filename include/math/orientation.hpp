...
#ifndef _H_MPL_GEOMETRY_ORIENTATION_H_
#define _H_MPL_GEOMETRY_ORIENTATION_H_
  ...
  namespace orientation {
    ...
    namespace detail {
      ...
      inline std::vector<cv::Point> extractNonZeroPixels(const cv::Mat & binaryMask) {
        ...
        pts.reserve((size_t)(cv::countNonZero(binaryMask)));
    ...
    inline orientation_t get(const cv::Mat & image, computationMethod method) {
      ...
      if(method == computationMethod::MOMENTS) {
        ...
        for(int i = 0; i < (int)(pts.size()); ++i) {
          dataPts.at<double>(i, 0) = (double)(pts[i].x);
          dataPts.at<double>(i, 1) = (double)(pts[i].y);
        }
    ...
    inline orientation_t get(const std::vector<cv::Point> & contour, computationMethod method) {
      ...
      if(method == computationMethod::MOMENTS) {
        ...
        cv::Mat dataPts((int)(contour.size()), 2, CV_64F);

        for(int i = 0; i < (int)(contour.size()); ++i) {
          dataPts.at<double>(i, 0) = (double)(contour[i].x);
          dataPts.at<double>(i, 1) = (double)(contour[i].y);
        }
...
#endif // _H_MPL_GEOMETRY_ORIENTATION_H_
