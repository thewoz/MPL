...
#ifndef _H_MPL_SOLVER_CPLEX_H_
#define _H_MPL_SOLVER_CPLEX_H_
...
//      model.add(data.getConstrains());
      ...
      if(!statusSolve) {
        ...
        fprintf(stderr, "mpl::cplex::minimize() error: solve failed\n");
        return false;
      }
...
//          
//          std::cerr << "error retrieving values: " << e.getMessage();
//          std::cout << "declared variables: " << data.getVariables().getSize() << std::endl;
//          std::cout << "variables active in the model: " << cplex.getNcols() << std::endl;
//          
...
//      counter++;
      ...
      if(cplex.getStatus() != IloAlgorithm::Optimal){
        fprintf(stderr, "mpl::cplex::minimize() error: optimal solution not found\n");
        return false;
      }
  ...
  bool minimizeSoft(std::vector<double> & coefficient, std::vector< std::vector<std::size_t> > & constrains, std::vector<bool> & solution, const char * output_file_name = NULL, double time_limit = 1.0e+75, double relative_optimality_tolerance = 1.0e-04, double absolute_optimality_tolerance = 1.0e-06) {
    ...
    for(std::size_t i=0; i<constrains.size(); ++i){
      ...
      } else { fprintf(stderr, "mpl::cplex::minimizeSoft() error: point without constraints\n"); abort(); }
